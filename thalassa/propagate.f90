...
subroutine DPROP_REGULAR(R0,V0,MJD0,tspan,tstep,tol,eqs,cart,int_steps,tot_calls,exitcode,mxpts,npts)
! Description:
...
use AUXILIARIES, only: MJDnext,MJDf,DU,TU
...
!real(dk),intent(in)  ::  tspan,tstep,tol,MJD0
double precision ,intent(in)  ::  tspan,tstep,tol,MJD0
integer,intent(in)  ::  eqs, mxpts
real(dk),intent(out)  ::  cart(mxpts*7)
!real(dk),intent(out)  ::  cart(mxpts,7)
integer,intent(out)   ::  int_steps,tot_calls,exitcode
! LOCALS
integer   ::  neq                  ! N. of equations
integer   ::  start
integer   ::  finish, mjdStart
...
! ==============================================================================
!write(*,*)"dprop args"
!write(*,*)R0, V0, tspan, tstep, eqs, x0, dx
...
MJDf = MJD0 + tspan
MJDnext = MJD0 + tstep
! write(*,*)"dprop: MJDf,MJD0,tstep,tspan",MJDf,MJD0,tstep,tspan

! Initialize state vector and independent variable
!write(*,*)"initialize vector", tol
!write(*,*)eqs,R0,V0,MJD0,MJDf,neq
call INIT_STATE(eqs,R0,V0,MJD0,neq,y0,x0)
!write(*,*)"init state",y0,x0
...
do ip=1,npts
    ...
    cart(mjdStart)     = MJD0 + t/TU/secsPerDay
    cart(start:finish) = CARTESIAN(eqs,neq,DU,TU,yx(ip,1),yx(ip,2:nels))
end do
...
!    else
!
!      tdif = abs(tcurr - tprev)
!
!      if (tdif >= 0.9*tstep ) then
!        mjdStart = 1 + (12*(idx - 1))
!        start = 2 + (12*(idx - 1))
!        finish = 7 + (12*(idx - 1))
!        cart(mjdStart)   = MJD0 + t/TU/secsPerDay
...
!        tprev = tcurr
!        idx = idx + 1
!      else if (tcurr > MJDf .AND. tprev < MJDf) then
!        mjdStart = 1 + (12*(idx - 1))
!        start = 2 + (12*(idx - 1))
!        finish = 7 + (12*(idx - 1))
!        cart(mjdStart)   = tprev
!        cart(start:finish) = CARTESIAN(eqs,neq,DU,TU,yx(ip-1,1),yx(ip-1,2:nels))
!        tprev = tcurr
!        idx = idx + 1
!      else if (tcurr < MJDf .AND. tprev > MJDf) then
!        mjdStart = 1 + (12*(idx - 1))
